            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    if (content.EndsWith("#"))
                    {
                        ...
                        var dirN1 = ConfigurationManager.AppSettings["dirN+1"];
                        ...
                        switch (token[0])
                        {
                            ...
                            case "tareas": // tareas#idCliente#Codprf#
                                ...
                                foreach (var tarea in tareas)
                                {
                                    ...
                                    string fileName = "";
                                    if (!new FileInfo(tarea.NameFile).Extension.Equals(".sql", StringComparison.OrdinalIgnoreCase))
                                    {
                                        fileName = dirVersiones + "\\" + tarea.Modulo + "\\" + tarea.NameFile;
                                    }
                                    else
                                    {
                                        String[] TipoScript = tarea.NameFile.Split('_');
                                        String Script = "";
                                        ...
                                        if(Script == "")
                                        {
                                            fileName = Path.Combine(dirN1,tarea.Modulo.Trim(), tarea.NameFile);
                                            eventLog1.WriteEntry("Estoy en Script == vacio", EventLogEntryType.Information, eventId);

                                        }
                                        else
                                        {
                                            fileName = Path.Combine(dirVersiones, tarea.Modulo, Script, tarea.NameFile);
                                            eventLog1.WriteEntry("Estoy en el else", EventLogEntryType.Information, eventId);

                                        }

                                        eventLog1.WriteEntry(fileName, EventLogEntryType.Information, eventId);
                                    }
                            ...
                            case "ambiente": //ambiente#idAmbiente#
                                var ambiente = ProcessMsg.Ambiente.GetAmbientebyID(int.Parse(token[1]));
                                json = JsonConvert.SerializeObject(ambiente);
                                Send(handler, json);
                                break;
                            ...
                            case "getversiones": // getversiones#idCliente#idAmbiente
                                ...
                                var lista= new List<ProcessMsg.Model.VersionBo> ();
                                if (idCliente == 435)
                                {
                                    lista = ProcessMsg.Cliente.GetVersionesAmbiente2(idCliente, idAmbiente, eventLog1);

                                }
                                else
                                {
                                    lista = ProcessMsg.Cliente.GetVersionesAmbiente(idCliente, idAmbiente, eventLog1);
                            ...
                            case "getcomponentes":
                                ...
                                {
                                    string fileName = dirVersiones + "\\" + archivo.Name;
                                    System.IO.FileInfo info = new System.IO.FileInfo(fileName);
                            ...
                            case "getfile":
                                ...
                                while (!bok)
                                {
                                    intentos++;
                                    byte[] buffer;
                                    if(token[1].Contains("SCRIPTS MAESTROS"))
                                    {
                                        buffer = ProcessMsg.Version.DownloadFile(token[1], int.Parse(token[2]), int.Parse(token[3]), dirN1, eventLog1);
                                        eventLog1.WriteEntry(String.Format("Contiene SCRIPTS MAESTROS", intentos), EventLogEntryType.Information, eventId);


                                    }
                                    else
                                    {
                                         buffer = ProcessMsg.Version.DownloadFile(token[1], int.Parse(token[2]), int.Parse(token[3]), dirVersiones, eventLog1);
