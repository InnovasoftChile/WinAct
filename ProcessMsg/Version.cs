...
namespace ProcessMsg
{
    public class Version
    {
        ...
        public static List<Model.VersionToClienteBo> GetVersionesToClientePub(int idCliente)
        {
            try
            {
                List<Model.VersionToClienteBo> lista = new List<Model.VersionToClienteBo>();
                var reader = new CnaVersionesCliente().ExecuteVersionesClientePub(idCliente);
                while (reader.Read())
                {
                    lista.Add(new Model.VersionToClienteBo
                    {
                        ...
                        Estado = char.Parse(reader["Estado"].ToString()),
                        FechaInstalacion = Convert.ToDateTime(reader["FecVersion"].ToString())
                    });
                }
                reader.Close();
                return lista;
        ...
        public static int DelVersionVC(int idVersion)
        {
            var query = new DelVersion();
            try
            {
                return query.ExecuteVC(idVersion);
            }
            catch (Exception ex)
            {
                var msg = "Excepcion Controlada: " + ex.Message;
        ...
        public static int DelVersionVCA(int idVersion)
        {
            var query = new DelVersion();
            try
            {
                return query.ExecuteVCA(idVersion);
            }
            catch (Exception ex)
            {
                var msg = "Excepcion Controlada: " + ex.Message;
        ...
        public static int DelControlCambios(int version, int tips, int modulo)
        {
            try
            {
                return new DelControlCambios().Execute(version, tips, modulo);
            }
            catch (Exception ex)
            {
                var msg = "Excepcion Controlada: " + ex.Message;
    ...
    , string.Format("{0:ddMMyyyyHHmmss}_{1}", DateTime.Now, txtInfo.Nombre)));
                        ...
                        ; if (process == 2)
                        {
                            ...
                            foreach (var ctrlCambios in listaCtrlCambios)
                            {
                                ...
                                {
                                    var pathtx = Path.Combine(pathVoN1,ctrlCambios.ModuloFmt.NomModulo, ctrlCambios.Tips.ToString()+".txt");
                                    StreamWriter sw = new StreamWriter(pathtx, true);
                                    sw.Close();
                                ...
                                "*****************************************************************"
                                    ...
                                    File.WriteAllLines(pathtx, bufferFinal.ToArray());
                                }
                                else
                                {
        ...
        public static int GenerarInstalador(int idVersion, string fileVersion, string dirSource, string dirN1, string dirFuentes)
        {
            try
            {
                ...
                using (System.IO.StreamWriter file = new System.IO.StreamWriter(dirSource + "Instalador\\" + fileVersion + ".iss", false, Encoding.GetEncoding("ISO-8859-1")))
                {
        ...
        public static Byte[] DownloadFile(string namefile, int posIni, int sizeMax, string dirVersiones, EventLog log)
        {
            try
            {
                ...
                if (new FileInfo(namefile).Extension.Equals(".sql", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    String[] TipoScript = sp[1].Split('_');
                    String Script = "";
                    if (TipoScript[0] == "Alt" || TipoScript[0] == "alt")
                    {
                        Script = "Alters";
                    ...
                    fileName = Path.Combine(dirVersiones,sp[0],Script,sp[1]);
                }
                else
                {
