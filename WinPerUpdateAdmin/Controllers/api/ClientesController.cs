...
namespace WinPerUpdateAdmin.Controllers.api
{
    public class ClientesController : ApiController
    {
        ...
        [Route("api/Cliente/{idCliente:int}/VersionesPublicadas")]
        [HttpGet]
        [Authorize]
        public Object GetVersionesClientesPub(int idCliente)
        {
            try
            {
                if (HttpContext.Current.Session["token"] == null) return Redirect(Request.RequestUri.GetLeftPart(UriPartial.Authority));
                return ProcessMsg.Version.GetVersionesToClientePub(idCliente);
            }
            catch (Exception ex)
            {
                throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.ExpectationFailed, ex.Message));
