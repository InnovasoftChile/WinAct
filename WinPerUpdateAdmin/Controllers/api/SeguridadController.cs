...
namespace WinPerUpdateAdmin.Controllers.api
{
    public class SeguridadController : ApiController
    {
        ...
        #region delete
        [Route("api/Perfil/DeletePerfil/{idUsuario:int}")]
        [HttpDelete]
        public Object DelPerfil(int idUsuario)
        {
            try
            {
                if (HttpContext.Current.Session["token"] == null) return Redirect(Request.RequestUri.GetLeftPart(UriPartial.Authority));
                HttpResponseMessage response = new HttpResponseMessage(HttpStatusCode.Created);
                var result = ProcessMsg.Seguridad.DelUsuario(idUsuario);
                if (result == 1)
                {
                    if(ProcessMsg.Seguridad.DelPersona(idUsuario)== 1)
                    {
                        response.StatusCode = HttpStatusCode.Accepted;
                        return Content(HttpStatusCode.OK, (object)null); ;
                    }
                    else
                    {
                        response.StatusCode = HttpStatusCode.BadRequest;
